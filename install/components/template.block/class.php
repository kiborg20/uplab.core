...
class TemplateBlock extends CBitrixComponent
{
	...
	public function onPrepareComponentParams($params)
	{
		...
		$isRedirect = $this->checkIfPageRedirected();
		...
		// Отложенные компоненты не отрабатывают при аяксе, а также при редиректах,
		// чтобы не нарушать работу страницы
		$params["DISABLE_COMPONENT_EXECUTE"] = "N";
		if ($params["DELAY"] == "Y" && ($isAjaxRequest || $isRedirect)) {
			$params["DISABLE_COMPONENT_EXECUTE"] = "Y";
		}

		if ($params["DISABLE_COMPONENT_EXECUTE"] != "Y") {
	...
	{
		if ($this->arParams["DISABLE_COMPONENT_EXECUTE"] == "Y") return false;
		...
		try {
			...
			if ($this->arParams["DELAY"] == "Y") {
				...
				$GLOBALS["APPLICATION"]->AddBufferContent(function () use ($component) {
					if ($component->checkIfPageRedirected()) {
						return "";
					}

					ob_start();
					call_user_func([$component, "executeTemplate"]);
				...
				if (file_exists($_SERVER["DOCUMENT_ROOT"] . $this->__path . '/component.php'))
				{
	 ...
	 * @noinspection PhpMissingReturnTypeInspection
	 */
	private function checkIfPageRedirected()
	{
		return ($GLOBALS["DELAY_EXECUTE_REJECT"] ?? "") == "Y";
	}
}
