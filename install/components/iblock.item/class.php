...
class IblockItemComponent extends CBitrixComponent implements Controllerable
{
	...
	public function onPrepareComponentParams($params)
	{
		...
		$result = array(
			...
			"SHOW_COUNTER"       => ($params["SHOW_COUNTER"] == "Y" ? "Y" : "N"),
		...
		if (!empty($this->arFilter) || !empty($this->arSort) || !empty($this->arParams["IBLOCK_ID"])) {
			...
			$arItem = IblockHelper::getList(
				[
					...
					"urlTemplates"  => [
						"DETAIL_PAGE_URL"  => $this->arParams["DETAIL_URL"],
						"SECTION_PAGE_URL" => $this->arParams["SECTION_URL"],
						"LIST_PAGE_URL"    => $this->arParams["LIST_URL"],
					],
				],
				false
			);
			...
			if (!empty($arItem)) {
				...
				$this->arResult["META_TITLE"] = strip_tags($arItem["~IPROPERTY_VALUES"]["ELEMENT_META_TITLE"] ?? "");
				...
				if ($this->arParams["SHOW_COUNTER"] === "Y") {
					$this->arResult["SHOW_COUNTER"] = [
						"ATTR" => " data-up-iblock-item-counter='{$arItem["IBLOCK_ID"]}:{$arItem["ID"]}' ",
					];
				}
			}
	...
	protected function executeEpilog()
	{
		...
		if (!empty($this->arParams["SHOW_COUNTER"] === "Y") && $this->arResult["ID"]) {
			Main\Page\Asset::getInstance()->addJs("/local/modules/uplab.core/install/components/iblock.item/show.counter.js");
	 ...
	 * @noinspection PhpUnused
	 */
	public function getCounterAction($iblockId, $elementId)
	{
