 ...
 *  Uplab\Core\Iblock\CopyIblock::iblock(2)->withContent()->toType('news');
 *
 * </pre>
 *
 * <pre>
 ...
 *  Uplab\Core\Iblock\CopyIblock::iblock(2)->toType('news');
 *
 * </pre>
 *
...
class CopyIblock
{
	...
	/**
	 * Возвращает инстанс класса self для дальнейших операций с ним.
	 *
	 * @param $iblockId
	 *
	 * @return self
	 */
	public static function iblock($iblockId)
	{
		if (!intval($iblockId)) return null;

		$instance = new self();
		$instance->SetIblockId($iblockId);
	 ...
	 * @return self
	 */
	public static function sections(array $sectionIds)
	{
		if (!is_array($sectionIds)) throw new Exception('Section ids should be an array');
		if (!count($sectionIds)) throw new Exception('Section ids array could not be empty');
		$sectionIds = array_filter($sectionIds, function ($val) {
			return intval($val);
		});

		if (!count($sectionIds)) throw new Exception('Section ids elements should be only integers.');

		/** @noinspection PhpDynamicAsStaticMethodCallInspection */
		$res = CIBlockSection::GetById(current($sectionIds))->Fetch();
		$iblockId = $res['IBLOCK_ID'];

		$instance = new self();
		$instance->SetIblockId($iblockId);
		$instance->SetSectionIds($sectionIds);
	 ...
	 * Если необходимо скопировать инфоблок программным способом, воспользуйтесь методом
	 * Uplab\Core\Iblock\CopyIblock::iblock().
	 *
	 * @param      $ID
	 * @param      $type
	 ...
	 * @param array $sectionIds
	 */
	private function SetSectionIds(array $sectionIds)
	{
		$this->sectionIds = $sectionIds;
	}

	private function syncIblockContent(
		$FROM_IBLOCK_ID,
		$TO_IBLOCK_ID,
		$arProperties,
		$arPropertyEnums,
		...
		/** @noinspection PhpDynamicAsStaticMethodCallInspection */
		if (Loader::includeModule("catalog") && CCatalog::GetByID($FROM_IBLOCK_ID)) {
		...
		while ($resSection = $rsSection->Fetch()) {
			...
			foreach (array("PICTURE", "DETAIL_PICTURE") as $code) {
				if (is_set($resSection, $code) && intval($resSection[$code]) > 0) /** @noinspection PhpDynamicAsStaticMethodCallInspection */ {
		...
		while ($obElement = $rsElement->GetNextElement()) {
			...
			foreach (array("PREVIEW_PICTURE", "DETAIL_PICTURE") as $code) {
				if (is_set($resElement, $code) && intval($resElement[$code]) > 0) /** @noinspection PhpDynamicAsStaticMethodCallInspection */ {
	...
	private function syncIblockProperties(
		$FROM_IBLOCK_ID,
		$TO_IBLOCK_ID,
		&$arProperties = array(),
		&$arPropertyEnums = array(),
		&$arUFProperties = array(),
		...
		while ($resData = $rsData->Fetch()) {
			...
			/** @noinspection PhpDynamicAsStaticMethodCallInspection */
			$rsDataExists = CUserTypeEntity::GetList(array(), array(
					"ENTITY_ID"  => $resData["ENTITY_ID"],
			...
			if (count($resEnumsOld) == count($resEnumsOld) && !empty($resEnumsOld)) {
				$arUFPropertyEnums = (array)$arUFPropertyEnums;
				$arUFPropertyEnums += (array)array_combine((array)$resEnumsOld, (array)$resEnumsNew);
			}
		...
		foreach ($arUpdateProperties as $arProperty) {
			...
			if ($ar = $rs->Fetch()) {
				...
				if (!($ID = $ibp->Add($arProperty))) {
					/** @noinspection PhpUndefinedFieldInspection */
					$APPLICATION->ThrowException(__METHOD__ . ': ' . $ibp->LAST_ERROR);
				}
			...
			foreach ($arUpdateEnums as $arEnum) {
				...
				if ($ar = $rs->Fetch()) {
					...
					if (!($ENUM_ID = $ibpenum->Add($arEnum))) {
						$APPLICATION->ThrowException(__METHOD__ . ": IB_DEMO_PROP_ADD_L_TYPE_ERR");
					}
