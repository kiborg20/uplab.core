...
class Helper
{
	...
	public static function getList($params, $cache = true, &$res = null)
	{
		...
		if (!empty($params["urlTemplates"])) {
			$res->SetUrlTemplates(
				($params["urlTemplates"]["DETAIL_PAGE_URL"] ?? "") ?: "",
				($params["urlTemplates"]["SECTION_PAGE_URL"] ?? "") ?: "",
				($params["urlTemplates"]["LIST_PAGE_URL"] ?? "") ?: ""
			);
		}
		...
		while ($ob = $res->GetNextElement(true, $useTilda)) {
			...
			if (!empty($item["DATE_ACTIVE_FROM"])) {
				$item["ACTIVE_FROM_UNIX"] =
				$item["TIMESTAMP_ACTIVE_FROM"] =
					strtotime($item["DATE_ACTIVE_FROM"]);
			}
			if (!empty($item["DATE_ACTIVE_TO"])) {
				$item["ACTIVE_TO_UNIX"] =
				$item["TIMESTAMP_ACTIVE_TO"] =
					strtotime($item["DATE_ACTIVE_TO"]);
			}
	...
	public static function getSectionsList($params, $cache = true, &$res = null)
	{
		...
		if (isset($params["incCnt"])) {
			$incCnt = $params["incCnt"] === true;
		} else {
			$incCnt = true;
		}
		...
		$res = CIBlockSection::GetList($arOrder, $arFilter, $incCnt, $arSelect, $arNavParams);
		if ($params["returnObject"] === true) return $res;
	...
	public static function addElementEditButtons(
		...
		if (!$component instanceof CBitrixComponent) return false;
		if (!$GLOBALS["APPLICATION"]->GetShowIncludeAreas()) return false;
		if (empty($actions["edit"]) && empty($actions["delete"])) return false;

		$uniqueId = $element['ID'] . $component->randString();

		if (empty($element["EDIT_LINK"]) || empty($element["DELETE_LINK"])) {
			$arButtons = CIBlock::GetPanelButtons(
				$element["IBLOCK_ID"], $element["ID"], 0, array('SESSID' => false, 'CATALOG' => true)
			);
	 ...
	 * @return bool
	 */
	public static function addSectionEditButtons(
		$component,
		$section,
		&$attribute = null,
		$actions = ["edit" => true, "delete" => true]
		...
		$uniqueId = $section['ID'] . $component->randString();

		if (empty($section["EDIT_LINK"]) || empty($section["DELETE_LINK"])) {
			$arButtons = CIBlock::GetPanelButtons(
				$section["IBLOCK_ID"], 0, $section["ID"], array('SESSID' => false, 'CATALOG' => true)
			);
