...
class Helper
{
	...
	public static function getFileInfo($file, $options = array())
	{
		...
		if ($fileItem["PATH"] == Application::getDocumentRoot()) $fileItem["PATH"] = false;
	...
	public static function resizeImage($picture, $params = array())
	{
		...
		if (empty($newPic) ||
			...
			$newPic = UplabCache::cacheMethod(function () use (
				...
				$defaultName = constant("DEFAULT_PICTURE") ?? false;
				// Пытаемся получить данные из модуля для случая, если константы модуля не дефайнятся
				if (!$defaultName) {
					$defaultName = Constant::get("DEFAULT_PICTURE");
				}

				$defaultSrc = $defaultName ? Application::getDocumentRoot() . $defaultName : false;
				$newPic = [];

				if ($defaultSrc && file_exists($defaultSrc)) {
					$ext = mb_strtolower(self::getFileInfo($defaultSrc)["EXT"]);
					$defaultPictureName = sprintf($defaultPictureName, $ext);
