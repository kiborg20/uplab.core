...
use CForm;
use Exception;
use Uplab\Core\Entities\Form\Form;
...
class Constant
{
	...
	function __construct()
	{
		...
		if(!is_dir($this->constantsDir) && !mkdir($concurrentDirectory = $this->constantsDir, 0777, true) && !is_dir($concurrentDirectory)) {
			AddMessage2Log(sprintf('Directory "%s" was not created', $concurrentDirectory));
		}
	...
	private function prepareConstantsIblockBySite()
	{
		...
		// Проверка: поддерживает ли версия битрикс работу с инфоблоками как с сущностями
		// См. подробнее https://dev.1c-bitrix.ru/learning/course/index.php?COURSE_ID=43&CHAPTER_ID=012864
		if (is_callable('\Bitrix\Iblock\Iblock::wakeUp')) {
			$selectFields["IBLOCK_ENTITY"] = "IBLOCK.API_CODE";
		}
	...
	private function prepareConstantsForms()
	{
		...
		if (Loader::includeModule("form")) {
			if (!class_exists(__NAMESPACE__ . "\\Entities\\Form\\Form")) return;
			if (!class_exists(FormTable::getObjectClass())) return;
	...
	private function makeConstantsFileForIDE()
	{
		$resultArray = [
			...
			" *",
			" * Константы, определенные в этом файле носят информационный характер",
